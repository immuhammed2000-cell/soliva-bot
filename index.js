...
function saveData() {
  try {
    const data = { ticketRoles, ticketCategories, ticketChannelRoles, ticketLogChannel };
...
async function openTicket(guild, user) {
  ...
  ticketInfo[ticketChannel.id] = { openerId: user.id, categoryName: 'عام', openedAt: Date.now() };
...
async function openTicketCategory(guild, user, category) {
  ...
  await ticketChannel.send({ content: `${user} ${roleMentions}`.trim(), embeds: [embed], components: [row] });
  ticketInfo[ticketChannel.id] = { openerId: user.id, categoryName: category.name, openedAt: Date.now() };
  return { channel: ticketChannel, existing: null };
}
...
client.on('guildMemberAdd', async member => {
  ...
  const embed = new EmbedBuilder()
    .setColor('#00ff00')
    .setTitle('🎉 عضو جديد ينضم إلينا')
    .setDescription(`أهلًا وسهلًا بك ${member} في سيرفر **${member.guild.name}**!`)
    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
    .addFields(
      { name: '👤 الاسم', value: member.user.tag, inline: true },
      { name: '🔢 ترتيب الانضمام', value: `${member.guild.memberCount}`, inline: true },
...
client.on('guildMemberRemove', async member => {
  ...
  const embed = new EmbedBuilder()
    .setColor('#ff0000')
    .setTitle('👋 مغادرة أحد الأعضاء')
    .setDescription(`نودّع **${member.user.tag}**، ونتمنى له كل التوفيق.`)
    .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
    .setTimestamp();
  channel.send({ embeds: [embed] });
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.customId === 'close_ticket') {
    await interaction.reply({ embeds: [new EmbedBuilder().setColor('#ff0000').setDescription('🔒 سيتم إغلاق التذكرة خلال خمس ثوانٍ...')] });
